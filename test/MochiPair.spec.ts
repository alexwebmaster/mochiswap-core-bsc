...
describe('MochiPair', () => {
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount.add(1), 0, wallet.address, '0x', overrides)).to.be.revertedWith('MochiSwap: K')
  ...
  it('feeTo:on', async () => {
    ...
    // expect(await pair.totalSupply()).to.eq(MINIMUM_LIQUIDITY.add('499501123255431'))
    // expect(await pair.balanceOf(other.address)).to.eq('499501123256431')
